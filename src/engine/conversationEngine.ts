...
export interface ConversationEngineOptions {
  ...
  onSelection?: SelectionCallback | null;
  onUsage?: (modelDisplayName: string, usage: UsageData) => void;
  maxTokensProvider?: (modelIndex: number) => number;
  exploreSettingsProvider?: () => ExploreModeSettings;
  customModelResolver?: (modelIndex: number) => string | null;
}
...
export async function generateModelResponse(
  ...
  seed?: number,
  usageCallback?: (modelDisplayName: string, usage: UsageData) => void,
  resolveCustomModel?: (modelIndex: number) => string | null
  ...
  if (company === 'hyperbolic_completion') {
    ...
    if (
      ...
      const resolved = resolveCustomModel(modelIndex);
      if (resolved) {
        apiName = resolved;
      }
    }
...
export class ConversationEngine {
  ...
  private maxTokensProvider?: (modelIndex: number) => number;
  private exploreSettingsProvider?: () => ExploreModeSettings;
  private customModelResolver?: (modelIndex: number) => string | null;
  ...
  constructor(options: ConversationEngineOptions) {
    ...
    if (Array.isArray(maxTokens)) {
      this.maxTokensPerModel = maxTokens;
    } else {
      // If a single number is provided, create an array with the same value for all models
      this.maxTokensPerModel = Array(models.length).fill(maxTokens);
    }
    ...
    this.usageCallback = onUsage || undefined;
    this.maxTokensProvider = maxTokensProvider;
    this.exploreSettingsProvider = exploreSettingsProvider;
    this.customModelResolver = customModelResolver;
  ...
  private getCurrentMaxTokens(modelIndex: number): number {
    const fallback = this.maxTokensPerModel[modelIndex] ?? 512;
    const override = this.maxTokensProvider
      ? this.maxTokensProvider(modelIndex)
      : undefined;
    const parsedOverride =
      override !== undefined && Number.isFinite(Number(override))
        ? Number(override)
        : undefined;
    const candidate = parsedOverride ?? fallback;
    return Math.max(1, Math.min(candidate, 1024));
  }
  ...
  private getCurrentExploreModeSettings(): ExploreModeSettings {
    const settings = this.exploreSettingsProvider
      ? this.exploreSettingsProvider()
      : this.exploreModeSettings;
    return { ...settings };
  }
